      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:px-6 lg:px-8 py-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-3.5 py-2 bg-teal-700 hover:bg-teal-800 text-white font-semibold text-xs rounded-lg shadow-xs transition self-start sm:self-auto"
          >
            <Plus size={14} />
        ...
        {/* Search Bar */}
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs mb-6 flex gap-3">
          <form onSubmit={handleSearchSubmit} className="flex-1 flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-8.5 pr-3 py-1.5 bg-white border border-slate-300 rounded-lg text-xs focus:ring-1 focus:ring-slate-900 focus:outline-none"
              />
            </div>
            <button
              type="submit"
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead className="bg-slate-50 text-slate-600 font-bold border-b border-slate-200 uppercase text-[10px] font-display">
                  <tr>
                    ...
                    <th className="py-2.5 px-4">Doctor / Prescriber</th>
                    <th className="py-2.5 px-4">Address</th>
                    <th className="py-2.5 px-4 text-center">Total Invoices</th>
                      ...
                      <td className="py-3 px-4 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          ...
                          <button
                            ...
                            className="px-2 py-1 bg-white hover:bg-slate-50 border border-slate-300 rounded text-xs font-medium text-slate-600 transition"
                          >
                            <Edit size={12} />
                          </button>
                        </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-5 border border-slate-200">
            <h3 className="text-sm font-bold text-slate-900 mb-3.5 font-display">
              {selectedCustomer ? 'Edit Patient Details' : 'Register New Patient'}
            </h3>

            {formError && (
              <div className="mb-3.5 p-2.5 bg-red-50 text-red-700 text-xs rounded-lg border border-red-200">
                {formError}
              </div>
            )}
            ...
            <form onSubmit={handleSaveCustomer} className="space-y-3 text-xs">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-1.5 bg-white border border-slate-300 rounded-lg font-mono text-slate-900 focus:ring-1 focus:ring-slate-900 focus:outline-none"
                />
              </div>
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 flex justify-end gap-2">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-4 py-1.5 bg-teal-700 hover:bg-teal-800 text-white rounded-lg text-xs font-bold shadow-xs"
                >
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[85vh] flex flex-col overflow-hidden border border-slate-200">
            <div className="px-5 py-3.5 border-b border-slate-200 flex justify-between items-center bg-slate-50">
              <div>
                <h3 className="font-bold text-slate-900 text-sm font-display">{activeHistory.name} — Order History</h3>
                <p className="text-[11px] text-slate-500 font-mono">{activeHistory.phone}</p>
              </div>
              <button
            ...
            <div className="p-5 overflow-y-auto divide-y divide-slate-100 space-y-2.5">
              {activeHistory.sales.length === 0 ? (
                <p className="text-xs text-slate-400 text-center py-6">No previous orders for this patient.</p>
              ) : (
                activeHistory.sales.map((sale) => (
                  <div key={sale.id} className="pt-2.5 first:pt-0">
                    ...
                    <div className="bg-slate-50 p-2 rounded text-[11px] space-y-0.5 border border-slate-100">
