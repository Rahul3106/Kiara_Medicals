    ...
    <div className="min-h-screen bg-slate-50/50 text-slate-900 flex flex-col font-sans">
      ...
      <main className="flex-1 w-full mx-auto px-6 sm:px-8 lg:px-12 py-6 space-y-6">
        ...
        <div className="bg-white/90 backdrop-blur-sm border border-slate-200 rounded-2xl p-6 shadow-sm">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 text-sm text-slate-600 font-mono mt-0.5">
                ...
                <span className="text-slate-300">•</span>
                <span className="flex items-center gap-2">
                  <MapPin size={14} className="text-slate-400 font-sans" />
            ...
            <div className="flex items-center gap-3 self-start md:self-auto">
              <Link
                to="/store/sales/new"
                className="inline-flex items-center gap-2.5 px-4.5 py-3 bg-teal-700 hover:bg-teal-800 text-white font-semibold text-sm rounded-lg transition shadow-2xs"
              >
                <Receipt size={18} />
                <span>POS Quick Bill</span>
                <span className="text-xs font-mono bg-teal-800/80 px-2 py-0.2 rounded border border-teal-600/60 ml-0.5">
                  F2
                </span>
              </Link>
              <Link
                to="/store/purchases/new"
                className="inline-flex items-center gap-2.5 px-4 py-3 bg-white hover:bg-slate-50 text-slate-700 hover:text-slate-900 border border-slate-300 rounded-lg text-sm font-semibold transition shadow-2xs"
              >
                <ArrowDownToLine size={18} />
                <span>Inward Stock</span>
                <span className="text-xs font-mono bg-slate-100 px-2 py-0.2 rounded border border-slate-200 text-slate-500 ml-0.5">
                  F4
                </span>
              </Link>
            </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-2xs">
          ...
          <div className="px-4 py-4 border-b border-slate-200 flex flex-col sm:flex-row gap-3.5 items-center justify-between bg-slate-50/70">
            <div>
            ...
            <div className="w-full sm:w-72 relative">
              ...
              <input
                ...
                className="w-full pl-8.5 pr-3 py-1.5.5 text-sm bg-white border border-slate-300 rounded-lg text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-slate-900 font-medium"
              />
            </div>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm text-slate-700">
                <thead className="bg-slate-50 text-sm uppercase font-bold text-slate-600 border-b border-slate-200 font-display">
                  <tr>
                    <th className="px-4 py-3.5">Medicine & Salt Formulation</th>
                    <th className="px-4 py-3.5 text-center">Batch</th>
                    <th className="px-4 py-3.5 text-center">Expiry</th>
                    <th className="px-4 py-3.5 text-center">Rack</th>
                  ...
                  {inventory.map((b) => {
                    ...
                    return (
                      <tr key={b.id} className="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td className="px-4 py-3.5 text-center font-mono text-sm text-slate-700 whitespace-nowrap">
                          {exp.toLocaleDateString('en-IN', { month: 'short', year: 'numeric' })}
                        </td>
                        <td className="px-4 py-3.5 text-center font-mono text-slate-600 text-sm">
                          {b.rackLocation || '—'}
                        </td>
                        <td className="px-4 py-3.5 text-right font-mono font-bold text-slate-900 text-sm">
                          {b.quantity} <span className="text-xs font-normal text-slate-400 font-sans">{b.medicine?.unit || 'Units'}</span>
                        </td>
                        <td className="px-4 py-3.5 text-right font-mono font-bold text-slate-900 text-sm">
                        ...
                        <td className="px-4 py-3.5 text-center">
                          {isExpired ? (
                            <span className="inline-flex items-center gap-2 text-sm font-semibold text-red-800 bg-red-50 border border-red-200 px-3 py-1 rounded">
