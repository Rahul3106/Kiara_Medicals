...
export const InventoryList = () => {
  ...
  const renderExpiryBadge = (batch) => {
    ...
    if (status === 'EXPIRED') {
      return (
        <span className="inline-flex items-center gap-2 px-3 py-1 text-xs font-semibold uppercase rounded bg-red-50 text-red-700 border border-red-200">
    ...
    if (status === 'CRITICAL') {
      return (
        <span className="inline-flex items-center gap-2 px-3 py-1 text-xs font-semibold uppercase rounded bg-amber-50 text-amber-800 border border-amber-200">
    ...
    if (status === 'NEAR_EXPIRY') {
      return (
        <span className="inline-flex items-center gap-2 px-3 py-1 text-xs font-semibold uppercase rounded bg-yellow-50 text-yellow-800 border border-yellow-200">
    ...
    return (
      <span className="inline-flex items-center gap-2 px-3 py-1 text-xs font-semibold rounded bg-emerald-50 text-emerald-800 border border-emerald-200">
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans">
      ...
      <main className="w-full mx-auto px-6 sm:px-8 lg:px-12 py-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-4">
            <Link
              ...
              className="inline-flex items-center gap-3 px-4.5 py-3 bg-teal-700 hover:bg-teal-800 text-white text-sm font-semibold rounded-lg shadow-xs transition"
            >
        ...
        {/* Toast Alert */}
        {toastMessage && (
          <div className="mb-6 p-4 bg-emerald-50 border border-emerald-200 text-emerald-800 text-sm rounded-lg flex items-center justify-between shadow-2xs">
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-xs mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          {/* Search Input */}
          ...
          <div className="flex items-center gap-2 overflow-x-auto text-sm font-semibold">
            <button
              ...
              className={`px-4 py-1.5.5 rounded-lg transition ${
                filter === 'all'
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-xs">
          <div className="px-5 py-4.5 border-b border-slate-200 flex items-center justify-between bg-slate-50/50">
            ...
            <span className="text-sm text-slate-500">Sorted by FEFO (Earliest Expiry First)</span>
          </div>

          {loading ? (
            <div className="text-center py-16 text-slate-400 text-sm font-mono">Loading branch inventory...</div>
          ) : inventory.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm text-slate-700">
                <thead className="bg-slate-50 text-sm uppercase font-bold text-slate-600 border-b border-slate-200 font-display">
                  <tr>
                    <th className="px-4 py-4">Medicine & Formulation</th>
                    <th className="px-4 py-4">Batch Number</th>
                    ...
                    <th className="px-4 py-4 text-right">Available Qty</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {inventory.map((batch) => (
                    <tr key={batch.id} className="hover:bg-slate-50/80 transition">
                      ...
                      <td className="px-4 py-4">
                        <span className="font-mono text-sm font-semibold bg-slate-100 text-slate-800 px-2 py-1 rounded border border-slate-200">
                          {batch.batchNumber}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-4">{renderExpiryBadge(batch)}</td>
                      <td className="px-4 py-4 text-sm font-mono text-slate-600">
                        {batch.rackLocation || '—'}
                      </td>
                      <td className="px-4 py-4 text-right text-sm font-mono text-slate-500">
