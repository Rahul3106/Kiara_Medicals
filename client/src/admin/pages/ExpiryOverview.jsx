      ...
      <main className="flex-1 w-full w-full mx-auto px-6 sm:px-8 lg:px-12 py-6 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <button onClick={fetchData} className="self-start sm:self-auto inline-flex items-center gap-2.5 px-4 py-1.5.5 bg-white hover:bg-slate-50 border border-slate-300 rounded-lg text-sm font-semibold text-slate-700 transition shadow-2xs">
            <RotateCw size={18} className={loading ? 'animate-spin' : ''} /> <span>Refresh Data</span>
          </button>
        </div>

        {/* Alert Strip: Compact padding with confident typography */}
        <div className="bg-white border border-slate-200 rounded-xl px-4 py-4 shadow-2xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <div className="text-sm font-bold uppercase tracking-wider text-slate-500 font-display">
            ...
            <div className="text-sm text-slate-500 mt-1 leading-tight">{batches.length} batches flagged within {daysFilter} days</div>
          </div>
          <div className="flex gap-4 border-t sm:border-t-0 sm:border-l border-slate-200 pt-2 sm:pt-0 sm:pl-5">
            {Object.entries(statusCounts).map(([status, count]) => (
              <div key={status} className="text-center">
                <div className={`text-2xl font-bold font-mono leading-none ${status === 'EXPIRED' ? 'text-red-700' : status === 'CRITICAL' ? 'text-amber-700' : 'text-slate-800'}`}>
                  {count}
                </div>
                <div className="text-xs font-bold uppercase text-slate-500 font-display mt-1">{STATUS_LABELS[status]}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-2xs flex flex-col sm:flex-row gap-3.5 items-center justify-between">
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <select
              ...
              className="px-4 py-1.5.5 bg-white border border-slate-300 rounded-lg text-sm font-medium text-slate-800 focus:ring-1 focus:ring-slate-900 focus:outline-none w-full sm:w-auto"
            >
          ...
          <div className="flex gap-2 self-start sm:self-auto">
            {[30, 60, 90, 180].map((d) => (
              <button
                ...
                className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition ${daysFilter === d ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}
              >
                {d} Days
              </button>
            ))}
        ...
        {!loading && batches.length > 0 && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
            {pieData.length > 0 && (
              <div className="lg:col-span-5 bg-white border border-slate-200 rounded-xl p-4.5 shadow-2xs flex flex-col justify-between">
                <div>
                  <h2 className="text-sm font-bold text-slate-900 font-display uppercase tracking-wider mb-0.5">
                    Expiry Severity Breakdown
                  </h2>
                  <p className="text-sm text-slate-500 mb-2">Batch counts by urgency tier</p>
                </div>
            ...
            {branchBarData.length > 0 && (
              <div className={`${pieData.length > 0 ? 'lg:col-span-7' : 'lg:col-span-12'} bg-white border border-slate-200 rounded-xl p-4.5 shadow-2xs`}>
                <div className="mb-2">
                  <h2 className="text-sm font-bold text-slate-900 font-display uppercase tracking-wider">At-Risk Capital by Store</h2>
                  <p className="text-sm text-slate-500">Financial exposure per retail branch</p>
                </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-2xs">
          <div className="px-4 py-3.5 border-b border-slate-200 bg-slate-50/70 flex items-center justify-between">
            ...
            <span className="text-sm text-slate-500 font-medium">Sorted by earliest expiry date</span>
          </div>

          {loading ? (
            <div className="text-center py-16 text-slate-400 text-sm font-mono">Scanning batches...</div>
          ) : batches.length === 0 ? (
            <div className="text-center py-16 text-slate-500 text-sm">No batches expiring within {daysFilter} days.</div>
          ) : (
              ...
              <table className="w-full text-left text-sm text-slate-700">
                <thead className="bg-slate-50 text-sm uppercase font-bold text-slate-600 border-b border-slate-200 font-display">
                  <tr>
                    ...
                    <th className="px-4 py-3.5 text-right">At-Risk Value (₹)</th>
                  ...
                  {batches.map((b) => (
                    <tr key={b.id} className={`hover:bg-slate-50/80 transition-colors ${b.status === 'EXPIRED' ? 'bg-red-50/30' : ''}`}>
                      <td className="px-4 py-3.5">
                        <div className="font-semibold text-slate-900 text-sm leading-snug">{b.medicine?.name}</div>
                        <div className="text-sm text-slate-500">{b.medicine?.composition || b.medicine?.genericName}</div>
                      </td>
                      <td className="px-4 py-3.5 font-mono text-sm font-bold text-slate-800">{b.batchNumber}</td>
                      <td className="px-4 py-3.5">
                        <span className="font-mono text-sm font-semibold bg-slate-100 text-slate-700 px-3 py-1 rounded border border-slate-200">
                          {b.branch?.code}
                        </span>
                      </td>
                      <td className="px-4 py-3.5 font-mono text-sm text-slate-700 whitespace-nowrap">
                      ...
                      <td className="px-4 py-3.5 text-center font-mono font-bold text-sm">{b.quantity}</td>
                      <td className="px-4 py-3.5 text-right font-mono font-bold text-slate-900 text-base">{fmt(b.atRiskValuation)}</td>
                      <td className="px-4 py-3.5 text-center">
                        <span className={`text-sm font-semibold px-3 py-1 rounded border ${STATUS_STYLES[b.status]}`}>
