    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col font-sans">
      ...
      <main className="flex-1 w-full w-full mx-auto px-6 sm:px-8 lg:px-12 py-6 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <button className="self-start sm:self-auto inline-flex items-center gap-2.5 px-4 py-1.5.5 bg-white hover:bg-slate-50 border border-slate-300 rounded-lg text-sm font-semibold text-slate-700 transition shadow-2xs">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white border border-slate-200 rounded-xl px-4 py-4 shadow-2xs">
            ...
            <div className="text-sm text-slate-500 mt-1 leading-tight">Completed sales volume</div>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl px-4 py-4 shadow-2xs">
            ...
            <div className="text-sm text-slate-500 mt-1 leading-tight">CGST + SGST total</div>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl px-4 py-4 shadow-2xs">
            ...
            <div className="text-sm text-slate-500 mt-1 leading-tight">Generated tax invoices</div>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl px-4 py-4 shadow-2xs">
            ...
            <div className="text-sm text-slate-500 mt-1 leading-tight">Per-ticket basket size</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
          ...
          {pieData.length > 0 && (
            <div className="lg:col-span-4 bg-white border border-slate-200 rounded-xl p-4.5 shadow-2xs flex flex-col justify-between">
              <div className="flex items-center justify-between mb-1">
                <h2 className="text-sm font-bold text-slate-900 font-display uppercase tracking-wider">
                  Payment Channels
                </h2>
                <span className="text-sm text-slate-500 font-mono">{sales.length} sample</span>
              </div>
              ...
              <div className="flex flex-wrap gap-x-3 gap-y-1 justify-center border-t border-slate-100 pt-2">
                {pieData.map((d, i) => (
                  <div key={i} className="flex items-center gap-2.5 text-sm text-slate-700">
                    ...
                    <span className="font-mono text-sm font-bold text-slate-900">{d.value}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className={`${pieData.length > 0 ? 'lg:col-span-8' : 'lg:col-span-12'} bg-white border border-slate-200 rounded-xl p-4.5 shadow-2xs`}>
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-sm font-bold text-slate-900 font-display uppercase tracking-wider flex items-center gap-2.5">
                <Filter size={18} className="text-slate-400" />
                <span>Filters & Search</span>
              </h2>
              <span className="text-sm text-slate-500 font-medium">Refine ledger records</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
              <select
                value={branchFilter}
                onChange={(e) => { setBranchFilter(e.target.value); setPage(1); }}
                className="px-4 py-1.5.5 bg-white border border-slate-300 rounded-lg text-sm font-medium text-slate-800 focus:ring-1 focus:ring-slate-900 focus:outline-none"
              >
              ...
              <form onSubmit={handleSearch} className="flex gap-2.5">
                <input
                  ...
                  className="flex-1 px-4 py-1.5.5 bg-white border border-slate-300 rounded-lg text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-slate-900"
                />
                <button
                  type="submit"
                  className="px-4 py-1.5.5 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-sm font-semibold transition"
                >
                  <Search size={18} />
                </button>
              </form>
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-2xs">
          ...
          {loading ? (
            ...
            <div className="text-center py-16 text-slate-400 text-sm">No invoices found matching current filters.</div>
          ) : (
                ...
                <tbody className="divide-y divide-slate-100 text-base">
                  {sales.map((s) => (
                    <tr key={s.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="px-4 py-3.5 text-slate-600 font-mono text-sm whitespace-nowrap">
                      ...
                      <td className="px-4 py-3.5">
                        <div className="font-semibold text-slate-900 text-sm leading-snug">{s.customer?.name || 'Walk-in Customer'}</div>
                        {s.customer?.phone && (
                          <div className="text-sm text-slate-500 font-mono leading-tight">{s.customer.phone}</div>
                        )}
                      </td>
                      <td className="px-4 py-3.5 text-center font-mono text-sm text-slate-700">
                        {s._count?.items || 0}
                      </td>
                      ...
                      <td className="px-4 py-3.5 text-center">
                        <span className={`text-sm font-semibold px-3 py-1 rounded border ${STATUS_STYLES[s.status] || ''}`}>
                          {s.status}
                        </span>
                      </td>
          ...
          {totalPages > 1 && (
            <div className="px-4 py-3.5 border-t border-slate-200 flex items-center justify-between text-sm bg-slate-50/70">
              ...
              <div className="flex gap-2.5">
                <button
                  ...
                  className="px-4 py-1.5 bg-white border border-slate-300 rounded-md text-slate-700 disabled:opacity-40 font-semibold shadow-2xs hover:bg-slate-50 transition"
                >
                  Previous
                </button>
                <button
